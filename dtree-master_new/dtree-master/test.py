...
def entropy(trainY):
    ...
    for freq in freq.values():
        data_entropy += (-freq / trainY.shape[0]) * math.log(freq / trainY.shape[0], 2)
...
def gain(trainY):
    ...
    for i in range(0, len(freq.index)):
        val_prob =  freq.values[i]/ trainY.shape[0]
        data_subset = trainY[trainY == freq.index.values[i]]       #[record for record in data if record[attr] == val]
        subset_entropy += val_prob * entropy(data_subset)

    # Subtract the entropy of the chosen attribute from the entropy of the
    # whole data set with respect to the target attribute (and return it)
    return (entropy(trainY) - subset_entropy)


def majority_value(trainY):
    """
    ...
    """
    # data = data[:]
    return most_frequent(trainY)


def most_frequent(trainY):
    """
    ...
    # for val in trainY.unique():
    #     if lst.count(val) > highest_freq:
    #         most_freq = val
    #         highest_freq = lst.count(val)
    for i in range(0,len(trainY.unique())):
        if trainY.value_counts().values[i] > highest_freq:
            most_freq = trainY.value_counts().index.values[i]
            highest_freq = trainY.value_counts().values[i]
...
def choose_attribute(trainY, fitness):
    """
...
def create_decision_tree(trainX, trainY, fitness_func):
    """
    Returns a new decision tree based on the examples given.
    """
    # data = data[:]
    vals = trainY
    default = majority_value(trainY)
    ...
    if len(unique_non_null) == 1:
        ...
        for val in get_values(trainY, best):
            # Create a subtree for the current value under the "best" field
            subtree = create_decision_tree(
                get_examples(trainY, best, val),
                [attr for attr in attributes if attr != best],
                target_attr,
                fitness_func)
...
if __name__ == "__main__":
    ...
    # cols = [col for col in ca.columns if col not in ['outcode', 'stop_outcome']]
    train = ca.iloc[:, 0:4]
    test = ca[ca.columns[4]]

    trainX = train.iloc[:int(len(ca) * 0.8), :]
    testX = train.iloc[int(len(ca) * 0.8):, :]

    trainY = test[:int(len(ca) * 0.8)]
    testY = test[int(len(ca) * 0.8):]
    ...
    #
    # # Get the training and test data from the given files
    # training_data = get_data(trainX, attributess)
    # test_data = get_data(testY, attributes)

    # Create the decision tree
    attributes = trainX.columns
    dtree = create_decision_tree(trainX, trainY, gain)
